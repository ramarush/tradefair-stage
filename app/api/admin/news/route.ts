...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      const newsQuery = `
        SELECT 
          n.id, n.title, n.slug, n.excerpt, n.featured_image, n.status, n.published_at, n.created_at, n.updated_at,
      ...
      const newsResult = await client.query(newsQuery, queryParams);
  ...
  try {
    ...
    try {
      ...
      if (existingNews.rows.length > 0) {
        return NextResponse.json(
          { error: 'News with similar title already exists' },
          { status: 409 }
        );
      }
      ...
      const result = await client.query(
        `INSERT INTO news (title, slug, content, excerpt, featured_image, status, author_id, published_at) 
         ...
         RETURNING id, title, slug, excerpt, featured_image, status, published_at, created_at`,
        [title, slug, content, excerpt || null, featured_image || null, status || 'draft', decoded.userId, publishedAt]
      );
